...
using FingerPrintScanner.Helpers;

namespace FingerPrintScanner.Controls.UserControls.DeviceController
{
	public partial class FingerPrintControl : UserControl
	{
		...
		// To hold the handle of the form
							...
							if (RegisterCount >= REGISTER_FINGER_COUNT) {
								...
								if (zkfp.ZKFP_ERR_OK == ret) {
									...
									if (zkfp.ZKFP_ERR_OK == ret) {         // <--- ENROLL SUCCESSFULL
										...
										_api.SendFingerPrintTokenAsync(userIdTextBox.Text, fingerPrintTemplate);
...
//
							...
							if (bIdentify) {
								...
								string fingerPrintToken = (_api.GetFingerPrintTokenAsync(userIdTextBox.Text)).Result;
								RegTmp = zkfp.Base64String2Blob(fingerPrintToken);

								int ret = fpInstance.Match(CapTmp, RegTmp);
								if (0 < ret) {
									DisplayMessage(MessageManager.msg_FP_MatchSuccess + ret, true);
									_api.SendVerificationStatusAsync(userIdTextBox.Text, true);
									return;
								} else {
									DisplayMessage(MessageManager.msg_FP_MatchFailed + ret, false);
									_api.SendVerificationStatusAsync(userIdTextBox.Text, false);
									return;
								}
							}
							#endregion
		...
		{
			ClearImage();
			if (clearDeviceUser && !btnInit.Enabled)
				ClearDeviceUser();
			if (enableEnroll)
				btnEnroll.Enabled = true;
		}
